...
1. 商品数据库模型
   ...
   ![](.image/ruoyi-vue-pro-商品列表.png)
   - `price`:取单价最低的商品的价格
   - `market_price`:取单价最低的商品的市场价格
   - `cost_price`:取单价最低的商品的成本价格
   - `stock`:sku的库存总数
   - `activity_orders`:获得显示排序数组.e.g:[2,1,3]
2. 商品新增
   1. 校验分类、品牌
   2. 校验 SKU
      1. 单规格，赋予单规格默认属性
      2. 校验属性项存在(缺属性值存在校验)
      3. 校验，一个 SKU 下，没有重复的属性。(无重复属性,应该也就不会有重复属性值,属性值挂在属性下)
      4. 校验，每个 Sku 的属性值的数量，是一致的
      5. 校验，每个 Sku 之间不是重复的
   3. 初始化 SPU 中 SKU 相关属性
      - `price` `market_price` `cost_price` `stock`等的设置
   4. 插入spu
   5. 插入sku
3. 商品修改
   1. 校验分类、品牌
...
1. 快递公司数据库模型
   ...
   ![](.image/ruoyi-vue-pro-快递发货-快递公司.png)


##### 快递发货-运费模版
1. 运费模版数据库模型

   ![](.image/ruoyi-vue-pro-快递发货-运费模板.png)

2. 代码片段(对比老、新两个列表，找出新增、修改、删除的数据)
...
1. 自提门店数据库模型
   ...
   ![](.image/ruoyi-vue-pro-门店自提-门店管理.png)
2. 使用腾讯地图实现门店地址搜索和经纬度的录入
